...
export const getVitalSignData = async (id: string) => {
  ...
  const formattedData = data.map((record) => {
    const heartRates = record.heartRate.split('-').map((rate) => {
      return parseInt(rate.trim());
    });
  ...
  const totalSystolic = data?.reduce((sum, acc) => {
    return sum + acc.systolic;
  }, 0);
  const totalDiastolic = data?.reduce((sum, acc) => {
    return sum + acc.diastolic;
  }, 0);

  const totalValue1 = formattedData?.reduce((sum, acc) => {
    return sum + acc.value1;
  }, 0);
  const totalValue2 = formattedData?.reduce((sum, acc) => {
    return sum + acc.value2;
  }, 0);
