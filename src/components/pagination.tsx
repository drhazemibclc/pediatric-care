...
export const Pagination = ({ totalPages, currentPage, totalRecords, limit }: PaginationProps) => {
  ...
  return (
    <div className='text-gray600 mt-5 flex items-center justify-between p-4'>
      <Button
        size={'sm'}
        variant={'outline'}
        disabled={currentPage === 1}
        onClick={handlePrevious}
        className='rounded-md bg-slate-200 px-4 py-2 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50'
      >
        Prev
      </Button>
      {/* 10 - 9 = 11 - 20 of 20 */}
